...
def preparefeatureLabel(batch_graph, batch_label, batch_adjs, embed_dim:int = 512):
    ...
    batch_node_feat = torch.zeros(batch_size, max_node_num, embed_dim)
...
class Trainer(object):
    def __init__(self, n_class, embed_dim:int = 1024):
    ...
    def train(self, sample, model):
        node_feat, labels, adjs, masks = preparefeatureLabel(sample['image'], sample['label'], sample['adj_s'], embed_dim=self.embed_dim)
        pred,labels,loss = model.forward(node_feat, labels, adjs, masks)

        return pred,labels,loss

class Evaluator(object):
    def __init__(self, n_class, embed_dim:int = 1024, return_logits:bool = False):
        self.metrics = ConfusionMatrix(n_class)
        self.embed_dim = embed_dim
        ...
        if not graphcam_flag:
            with torch.no_grad():
                if not self.return_logits:
                    ...
                    pred, labels, loss, logits = model.forward(node_feat, labels, adjs, masks)
                    return pred, labels, loss, logits
        else:
